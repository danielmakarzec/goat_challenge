...
const InfluencerCard = ({ influencer, setSearchString }) => {
  ...
  return (
    <Card>
      ...
      <ContentContainer>
        ...
        <TagContainer>
          {<Tag onClick={() => setSearchString(influencer.primary_tag.name)}>{influencer?.primary_tag.name}</Tag>}
        </TagContainer>
        <FieldTitle>Secondary Tags</FieldTitle>
        <TagContainer>
          {influencer?.tags.map((tag) => (
            <Tag onClick={() => setSearchString(tag.name)} key={"inf_tag_key" + influencer.id + tag.id}>{tag.name}</Tag>
          ))}
        </TagContainer>
      </ContentContainer>
